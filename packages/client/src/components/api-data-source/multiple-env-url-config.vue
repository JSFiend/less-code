<template>
  ...
  <el-dialog v-model="dialogVisible" title="多环境配置url" append-to-body>
    <el-table :data="data.envUrl" class="w-full">
      <el-table-column prop="name" label="环境名" width="150px">
...
watch(dialogVisible, (newVisible) => {
  if (newVisible === true) {
    ...
    data.envUrl = props.envUrl.map((env) => {
      ...
      return assignWith({}, env, { url: props.url }, (objValue, srcValue) =>
        objValue ? objValue : srcValue
      );
    });
  }
});
